function Setup-Host {
    ...
    if ( ! ( ( $script:SkipAll ) -or ( $script:SkipDeps ) ) ) {
        ("sora-cpp-sdk", "boost", "lyra", "webrtc") | ForEach-Object {
            $_Dependency = $_
            ...
            $_Filename = $BuildSpec.dependencies."${_Dependency}"."windows-${script:Target}"
            $_Uri = "${_BaseUrl}/${_Version}/${_Filename}"
            $_Target = "sora-cpp-sdk"

            Log-Information "url=${_Uri}"
